function change_seg_background_to_blue_chan(indir,changebackbool)
...
%         load(allmatfiles{i})
        ...
        if ~changebackbool
            ...
            lastpart_filename = ['convertedsize_blue_T',zerobuff_t,num2str(tnum),'_Z',zerobuff_z,num2str(znum),'.tif'];

            folderblue = 'blue';
            new_blue_filename = {['..',filesep,folderblue,filesep,lastpart_filename]};
            
%             copyfile(lastpart_filename,[pwd,filesep,lastpart_filename]);

%             filenames = lastpart_filename;
            filenames = new_blue_filename;
