function multiple_barcharts_red(histcounts,binranges,legendtxt,fig_name_str,savedir,indnumsspecial)
        ...
        if   ~isempty(histcounts(ii).hist_perc_list)
            ...
            max_var_freq = max([currbins(:);max_var_freq]);
                ...
                if any(~isnan(mean(currbins,1)))
                    ...
                    line(xpos,[0 0.25],'color',min(colorlist(ii,:).*0.6,1),'linewidth',2);
...
%             custom xticks
            ...
            remove_ind = ~ismember([1:length(myxticklabels)],[2, 6, 11, 16, 21, 26, 31], 'legacy');
...
%              set(gca,'ylim',[0,0.25]);
                ...
                figurenamestring = [savedir,filesep,fig_name_str,'-red-as-line'];
                if ~isdir(savedir)
%                     fid = fopen('create_these_dirs.m','w');
%                     fprintf(fid, '%% ! sudo chmod 777 -R %s \n', upperdir);
%                     fprintf(fid, '!mkdir %s ', [figuredirstring]);                       
