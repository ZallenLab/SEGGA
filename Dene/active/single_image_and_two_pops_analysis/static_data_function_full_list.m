function [data, full_data, other_data, data_description] =...
    static_data_function_full_list(geom, sel_cells, light_data)
%
% INPUTS:
%
% geom -> geom data for a single frame
%
% passing sel_cells = 0 will analyze all cells, default is to analyze only
% those within the poly.
% 
...
% OUTPUTS:
%
% 'full_data' lists measurements of each individual cell.
%
% 'data' -> averages and std for the population of cells
%
% 'other_data' -> experimental measurements, not yet in the main pipeline
%
% calling static_data_function with no input parameter will return only a
% description of the data. Otherwise 'data_description' is empty
%
%
data_description = [];
if nargin < 1
    ...
    full_data = [];
    other_data = [];
    data_description = get_full_data_description_for_static_data_function();
...
full_data.qVal = total_length./realsqrt(areas);
...
%%% Not using distortion - old metric
...
full_data.int_ang = int_angs;

data.frac_int_ang150.avg = sum(int_angs > 150) ./ sum(int_angs > 0);
data.frac_int_ang150.std = 0;

[~, pd_full] = pattern_defo_graner(geom, sel_cells');
full_data.pd = pd_full';
