function vals  = check_for_edge_intensity_full_profiles(img, pt_one, pt_two, method, trap_bool, search_radius, buffer, flip)
...
%     val = img((m * (round(pt_one(1)-1))) + round(pt_two(2)));
    vals = get_part_from_matrix(img, round(pt_one(1)) , round(pt_two(2)));
    return
end
%%%%
...
vals = get_edge_intensity_full_profiles(img, sample_pixels, method, buffer);
