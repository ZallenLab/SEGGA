function mat = fill_nans_linear_special(mat, fill_l, fill_r, max_gap)
...
if nargin < 4 || isempty(max_gap)
    max_gap = 5;
end

for i= 1:length(mat(1, :))
    mat(:, i) = fill_nans(mat(:, i), fill_l, fill_r, max_gap);
end



function vec = fill_nans(vec, fill_l, fill_r, max_gap)
ind = isnan(vec);
if ~any(ind) || all(ind)
    return
end
...
for i = 1:2:(length(boundaries) - 1)
    ...
    if (boundaries(i+1) - boundaries(i))<max_gap
