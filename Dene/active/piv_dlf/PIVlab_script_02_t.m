...
choice = questdlg('Visualizing Slows Processing', ...
        'Display PIV?', ...
        'Visualize','Run Without Visualization','Cancel','Visualize');
% Handle response
switch choice
    case 'Visualize'
        visual_bool = true;
        pivFigH = figure;
    case 'Run Without Visualization'
        visual_bool = false;
    case 'Cancel'
        return
end
...
s{1,1}= 'Int. area 1';           s{1,2}=20;         % window size of first pass
s{2,1}= 'Step size 1';           s{2,2}=10;         % step of first pass
...
for i=1:(amount-1)
    ...
    if max(image1(:))>256
        image1 = cast(image1,'single');
        image1 = (image1/max(image1(:)))*256;
        image1 =cast(image1,'uint8');
    end
    if max(image2(:))>256
        image2 = cast(image2,'single');
        image2 = (image2/max(image2(:)))*256;
