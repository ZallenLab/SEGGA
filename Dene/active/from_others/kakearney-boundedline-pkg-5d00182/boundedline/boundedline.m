...
% [found, alpha, varargin] = parseparam(varargin, 'alpha');
...
%         ptchcol{iln} = plotdata{iln,5};
        whiteness = max(1-trans,0.3);
        ptchcol{iln} = interp1([0 1], [1 1 1; lncol{iln}], whiteness);
        alpha{iln} = trans;
    else
        lncol{iln} = plotdata{iln,5};
...
for iln = 1:nline
    hl(iln) = line(xl{iln}, yl{iln}, 'marker', marker{iln}, 'linestyle', lnsty{iln}, 'color', lncol{iln},'linewidth',linewidth);
end
