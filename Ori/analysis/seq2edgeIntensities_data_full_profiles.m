function [protein_levels background_levels x1 x2 y1 y2 cells_levels min_pixel] ...
    = seq2edgeIntensities_data_full_profiles(seq, protein_img_filename, edges, seg_filename, options, data, ...
                   cells_sel)
if nargin < 2 || isempty(protein_img_filename)
    [protein_img_filename, protein_img_file_path] = uigetfile('*.tif', 'Select images for intensity measurements (any timepoint)');
    if isequal(protein_img_filename, 0) && isequal(protein_img_file_path, 0)
        return
    end
    protein_img_filename = fullfile(protein_img_file_path, protein_img_filename);
end
if ischar(protein_img_filename)
    protein_img_filename = {protein_img_filename};
end
...
protein_levels = cell(length(seq.frames), length(edges), length(protein_img_filename));
background_levels = nan(length(seq.frames), length(edges), length(protein_img_filename));
% cells_sel = find(any(data.cells.selected));
inverse_cells_map = zeros(1, size(seq.cells_map, 2));
inverse_cells_map(cells_sel) = 1:length(cells_sel);
cells_levels = nan([length(seq.frames) length(cells_sel) length(protein_img_filename)]);
min_pixel = inf(length(seq.frames), length(protein_img_filename));
...
%         end
    ...
    for channel = 1:length(protein_img_filename)
        protein_img(channel).image = double(imread(put_file_nums(protein_img_filename{channel}, t, z_for_t(t))));
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for ed_num = 1:length(local_edges);
        ...
        for channel = 1:length(protein_img_filename)
            ...
            protein_levels{i, found_edges(ed_num), channel} = ...
                check_for_edge_intensity_full_profiles(img_minus_background, a([2 1]), b([2 1]),...
                options.method);
%             background_levels(i, found_edges(ed_num), channel) = ...
...
% protein_levels = protein_levels - background_levels;
