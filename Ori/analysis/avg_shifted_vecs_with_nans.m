function [avg std_err std_std num] = avg_shifted_vecs_with_nans(vecs, shifts, ...
    boundary_mode_l, boundary_mode_r, time_steps, global_timescale, avg_func)

if nargin < 4 || isempty(boundary_mode_l) || isempty(boundary_mode_r)
    boundary_mode_l = 2;
    boundary_mode_r = 2;
end

if any(strcmp(class(boundary_mode_l),{'char','string'}))
    boundary_mode_l = str2num(boundary_mode_l);
end

if any(strcmp(class(boundary_mode_r),{'char','string'}))
    boundary_mode_r = str2num(boundary_mode_r);
end

if ~isnumeric(boundary_mode_l)
    boundary_mode_l = 2;
end
if ~isnumeric(boundary_mode_r)
    boundary_mode_r = 2;
end
...
last = -inf;
for i = 1:length(vecs);
    timescale = ((1:length(vecs{i})) + shifts(i))*time_steps(i)/60;
