function figsH = plot_analysis_graphs(action, vars, dir_names, groups, ...
    home_dir, savedir, target_time_step, time_lim, figsH, text_argin,...
    shift_hack,unifydecimals,xtra_txt,ylim_input,save_filetypes,exec_str,...
    show_pvals)
% figsH = plot_analysis_graphs(action, vars, dir_names, groups, ...
%     home_dir, savedir, target_time_step, time_lim, figsH, text_argin,...
%     shift_hack,unifydecimals,xtra_txt,ylim_input,save_filetypes,exec_str)
% 
% action determines how to spread the different group-variable plots across
% figures. Accepted values are (for m variables, n groups, N total movies):
% 
% 'one_fig_per_group' - n charts with m lines on each (for each group show
% all variables); output files named after groups
% 'one_fig_per_var' - m charts with n lines on each (for each variable show
% all groups on one chart); output files named after vars
% 'one_fig' - 1 chart with n x m lines (show each group for each variable
% all on one chart); output file is call 'all-vars'
% 'single_movies_var' - m charts with N lines (a chart for each variable, a
% line for each movie in each chart); figure files are named after vars
% 'single_movies_group' - N charts with m lines (a chart for each movie, a
% line for each variable in each chart); figure files are named after movie
% directory names
% 
...
% DISCONTINUED - too many figures, only saving them and closing them now.
% 
...
% 
% FULL LIST, SHORT EXPLANATION OF INPUTS
% 1. action - type of figures produced (grouping)
% 2. vars - data variables to plot
% 3. dir_names - all directories to use
% 4. groups - groups (genotypes grouping directories)
% 5. home_dir - directory above 'dir_names'
% 6. savedir - location to save charts
% 7. target_time_step - time resolution of plots (leave: target_time_step = [])
% 8. time_lim - (x-axis limits)
% 9. figsH - preexisting figure handle(s) on which to draw plots
% 10. text_argin - extra text to put on chart
% 11. shift_hack - set shift to middle for showing cross correlation
% 12. unifydecimals - set same number of decimal places for all y ticklabels
% 13. xtra_txt - similar to text_argin, not active in code, don't use
% 14. ylim_input - set the y-axis limits (applies to all charts)
% 15. save_filetypes - filetypes to save charts as
% 16. exec_str - a string that will be evaluated before loading variables
% for each directory ('analyze_dir_new would rerun the analysis for that
% dir)
% 17. show_pvals - a boolean - true changes the chart to pvalues - using a
% students ttest and creating txt output for those pvals
% 
if ((nargin <17) || (isempty(show_pvals)))
    show_pvals = false;
end

if show_pvals
    if any(strcmp(action,{'single_movies_var','single_movies_group',...
            'one_fig'}))
        display('can only run ttest on comparisons of groups');
        show_pvals = false;
    end
end
...
for i = 1:length(vars)
    if ~isfield(vars, 'color') || isempty(vars(i).color)
        ...
        vars(1).color = [0 0.8 0];
        if length(vars)>3
            vars(4).color = [0.2 0.2 0.2];
        end
    end
...
[vecs shifts_returned dir_ind time_step_ret single_movie_dirs] = read_vecs_from_files(...
    home_dir, dir_names, vars, exec_str);
...
if strcmpi(action, 'single_movies_var') || strcmpi(action, 'single_movies_group')
    ...
    for i = 1:length(single_movie_dirs)
        ...
        major = 0;
        for j = 1:length(orig_groups)
            if any(ismember([orig_groups(j).dirs],iy(i)))
                    major = j;
            end
        end
        if major==0
            display('cannot find major group');
            return
        end
        if ~(isfield(orig_groups,'keepCol') && (orig_groups(major).keepCol))            
            groups(i).color = get_cluster_colors(i);
        else            
            groups(i).color = orig_groups(major).color;
        end
        groups(i).linespec = '';
    end
...
if show_pvals
    cntrl_ind = 1;
    if size(vecs(1),2)<2
        display('need at least two groups');
        pvals = [];
    end
end
...
switch lower(action)
    case {'one_fig_per_group', 'single_movies_group'}
        figsH = plot_all_vars_overlayed(avg_vecs, global_timescale, ...
            groups, vars, figsH, time_lim, savedir, text_argin,unifydecimals,...
            xtra_txt,ylim_input,save_filetypes);
    case {'one_fig_per_var', 'single_movies_var'}
        figsH = plot_by_group(avg_vecs, {vars.title}, groups, vars, ...
            global_timescale, figsH, time_lim, savedir, text_argin,unifydecimals,...
            xtra_txt,ylim_input,save_filetypes,show_pvals);
    case 'one_fig'
...
function [vecs shifts_returned dir_ind time_step_ret movie_dirs] = read_vecs_from_files(...
    home_dir, root_dir_names, vars, exec_str)
exec_str_bool = true;
if (nargin <4) || isempty(exec_str)
    exec_str_bool = false;
end
...
for i = 1:length(root_dir_names)
    ...
    for j = 3:length(dir_names)
        if dir_names(j).isdir
            ...
            %%% run a fresh analysis before loading info
            %%% or run anything you like for that matter
            if exec_str_bool
                display(pwd);
                eval(exec_str);
            end
            ...
            for k = 1:length(vars)
                if isfield(vars(k), 'force_rerun') && ~isempty(vars(k).force_rerun)
                    if vars(k).force_rerun
                        display('forcing a rerun of analysis...');
                        display(['var: ',vars(k).title]);
                        if isfield(vars(k), 'data_gen_func') && ~isempty(vars(k).data_gen_func)
                            display(['generating with : ' vars(k).data_gen_func]);
                            eval(vars(k).data_gen_func);
                        end
                    end
                end
                try 
                    d = load(vars(k).file_name, vars(k).var_name);
                catch %%% adding on new analyses
                    display(['could not load file: ' vars(k).file_name,' -> ', vars(k).var_name]);
                    if isfield(vars(k), 'data_gen_func') && ~isempty(vars(k).data_gen_func)
                        display(['generating with : ' vars(k).data_gen_func]);
                        eval(vars(k).data_gen_func);
                        d = load(vars(k).file_name, vars(k).var_name);
...
function figsH = plot_by_group(avg_vecs, titles, groups, vars, time_axis, ...
    figsH, time_lim, savedir, text_argin, unifydecimals,xtra_txt,...
    ylim_input,save_filetypes,show_pvals)
...
%     set(gca,'TickDir','out');
    bname = [savedir, titles{i}];
    
    if ~isdir(savedir)
        display(['creating save dir: ',savedir]);
        mkdir(savedir);
    end
...
function figsH = plot_all_vars_overlayed(avg_vecs, time_axis, groups, vars, ...
    figsH, time_lim, savedir, text_argin, unifydecimals,xtra_txt,ylim_input,save_filetypes)
...
%     ylim([-0.7 0.4]);
	...
	bname = [savedir, groups(j).label,'-all-vars',];
...
diffmat = ones(size(colors))*0.2;
diffmat(1:2:length(diffmat(:))) = diffmat(1:2:length(diffmat(:)))*-1;
colors = colors + diffmat;
colors = min(max(0,colors),1);
