function figsH = plot_analysis_graphs_soliderrbars_timegiven(action, vars, dir_names, groups, ...
    home_dir, save_dir, target_time_step, time_lim, figsH, text_argin,shift_hack,unifydecimals,xtra_txt,ylim_input)
...
[vecs shifts_returned dir_ind time_step_ret single_movie_dirs, xvals, chan_titles] = read_vecs_from_files_timegiven(...
...
avg_vecs = average_and_sum_vecs(vecs, shifts_returned, ...
    groups, dir_ind, time_step_ret, global_timescale, vars, xvals);


switch lower(action)
    case {'one_fig_per_group', 'single_movies_group'}
        figsH = plot_all_vars_overlayed(avg_vecs, global_timescale, ...
            groups, vars, figsH, time_lim, save_dir, text_argin,unifydecimals,chan_titles,xtra_txt,ylim_input);
    case {'one_fig_per_var', 'single_movies_var'}
        figsH = plot_by_group(avg_vecs, {vars.title}, groups, vars, ...
            global_timescale, figsH, time_lim, save_dir, text_argin,unifydecimals,chan_titles);
    case 'one_fig'
...
function [vecs shifts_returned dir_ind time_step_ret movie_dirs, xvals, chan_titles] = read_vecs_from_files_timegiven(...
    home_dir, root_dir_names, vars)

chan_titles = cell(length(root_dir_names),length(vars));
...
%    shrinking_edge_angles_len_corrected;

% %             DLF DEBUG TEMPORARY
	
%     if isempty(dir('shrink_angs_new*'))
%         
%         display('missing shrink data, running analysis');
%         display(pwd);
%         
%         linked_shrinking_edges_avgs;
% 
...
%     end
            ...
            for k = 1:length(vars)
                ...
                if isfield(vars(k), 'xvals_func') && ~isempty(vars(k).xvals_func)
                    d.xvals = vars(k).xvals_func(dfull.(vars(k).var_name), shift_info);
                end
                xvals{k, cnt} = d.xvals;
                
                if any((d.xvals==0))
                    shifts_returned(cnt) = -find((d.xvals==0));
                else
                    shifts_returned(cnt) = -give_sign_changes(d.xvals);
                end
                
                if isfield(vars(k), 'name_func') && ~isempty(vars(k).name_func)
                    if vars(k).get_name_from_chan
                        addendum = [vars(k).name_func(dfull.(vars(k).var_name), shift_info)];
                        chan_titles(i,k) = {[vars(k).start_title,' ',addendum]};
%                         display(addendum);
                    end
                end
...
function avg_vecs = average_and_sum_vecs(vecs, shifts, groups, dir_ind, ...
    time_steps, global_timescale, vars, xvals)
...
for i = 1:size(vecs, 1)
    for j = 1:length(groups)
        ...
        [avg_vecs(i).group(j).avg avg_vecs(i).group(j).std_err ...
            ...
            avg_shifted_vecs_with_nans_timemod({vecs{i, grouped_dir_ind(j, :)}}, ...
            shifts(grouped_dir_ind(j, :)), vars(i).boundary_l, ...
            vars(i).boundary_r, time_steps_group, global_timescale, vars(i).avg_func, {xvals{i, grouped_dir_ind(j, :)}});
...
function figsH = plot_by_group(avg_vecs, titles, groups, vars, time_axis, ...
    figsH, time_lim, save_dir, text_argin, unifydecimals,chan_titles)
...
%%%%%%%%%%%     debug/hack here down        
%         plot_adjust_shift = 0.003;
%         y_vals = avg_vecs(i).group(j).avg;
%         if j ==1
%         ind_vals = y_vals<=plot_adjust_shift;
%         y_vals = ind_vals*plot_adjust_shift+y_vals;
%         end
%%%%%%%%%%%     debug/hack here up
...
% %%%%%%%%%%%     debug/hack here up
    ...
    title([chan_titles{i}], 'fontsize', 16, 'fontweight', 'bold','interpreter','none')
...
function figsH = plot_all_vars_overlayed(avg_vecs, time_axis, groups, vars, ...
    figsH, time_lim, save_dir, text_argin, unifydecimals,chan_titles,xtra_txt,ylim_input)
% if nargin < 5 || isempty(colors)
...
%     ylim([-0.7 0.4]);
    ...
    if exist('chan_titles') && ~isempty(chan_titles)
        leghandle = legend(h(end:-1:1), chan_titles(j,:), 'fontweight', 'bold','interpreter', 'none',...
...
function figsH = plot_all_one_window(avg_vecs, time_axis, groups, vars, ...
    figsH, time_lim,save_dir, text_argin, unifydecimals)
