function convert_and_project_exp_images_dir_adaptive(dir_name, dest_dir, base_filename, min_z, max_z)
...
zbounds_list = zeros(length(files),2);
fid = fopen(fullfile(dest_dir, 'adaptive_z_bounds'), 'w');
fprintf(fid, 't zmin zmax\n');
for i = 1:length(files)
    ...
    [top, bottom] = find_top_of_embryo(files(i).name);
    zbounds_list(i,:) = [top, bottom];
    project_and_convert_exp_image(files(i).name, dest_dir, base_name, t_num, bottom, top);
    fprintf(fid, '%d %d %d\n',i,top, bottom);
end
fclose(fid);
